...
def capture_packets(packet):
    ...
    captured_packets.append(packet.summary())  # Agregar el resumen del paquete a la lista
...
def start_capture(interface, packet_count):
    ...
    try:
        ...
        generate_report()  # Generar el reporte al final de la captura
    except Exception as e:
        print(f"Error durante la captura de paquetes: {e}")  

# Función para generar un reporte de los paquetes capturados
def generate_report():
    with open('reporte_captura.txt', 'w') as report_file:
        report_file.write("Reporte de Captura de Paquetes\n")
        report_file.write("=" * 30 + "\n")
        for packet in captured_packets:
            report_file.write(f"{packet}\n")
    print("Reporte generado en 'reporte_captura.txt'.")
